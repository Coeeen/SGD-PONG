...
int main(int argc, char* argv[]) {
    ...
    if (!okno) {
        std::cerr << "Wystapil blad z wygenerowaniem okna gry" << SDL_GetError() << std::endl;
        SDL_Quit();
        return 1;
    }
    ...
    SDL_Surface* tlo =SDL_GetWindowSurface(okno);
    SDL_FillRect(tlo,NULL,SDL_MapRGB(tlo ->format, 0,0,0));
